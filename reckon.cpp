...
int main(){
    ...
    cin.tie(NULL);
    //
    vector<vector<vector<ll int>>>dp(50,vector<vector<ll int>>(101,vector<ll int>(50,0)));
    vector<vector<vector<ll int>>>ans(50,vector<vector<ll int>>(101,vector<ll int>(50,0)));
    ...
    for(int i =1;i<50;i++){
        for(int j =1;j<=100;j++){
            for(int k =0;k<=i;k++){
                ...
                if(k>=1) {dp[i][j][k]=(dp[i][j][k] +ans[i-1][j-1][k-1])%mod;}        
